...
const txtAnio =  document.querySelectorAll("#txtAnio");
for(let i=0; i<txtAnio.length;i++){
    const validarAnio = function (e){
        if(txtAnio[i].value.length == 0){
            let validarAnio = /^[2-9]{1}$/;
            let tecla = e.key;
            if(!validarAnio.test(tecla)){ e.preventDefault();}
        }
        else {
            let validarAnio = /^[0-9]{1}$/;
            let tecla = e.key;
            if(!validarAnio.test(tecla)){ e.preventDefault();}
        }
    };
    txtAnio[i].addEventListener("keypress", validarAnio);
}